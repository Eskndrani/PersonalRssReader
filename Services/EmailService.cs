...
public interface IEmailService
{
    ...
    Task SendFavoriteDigestAsync(string email, string userId, string baseUrl, List<ArticleEntity> articles);
}
...
public sealed class SmtpEmailService : IEmailService
{
    ...
    public async Task SendFavoriteDigestAsync(string email, string userId, string baseUrl, List<ArticleEntity> articles)
    {
        if (articles.Count == 0) return;

        var itemsHtml = string.Join("", articles.Select(a =>
            $"""<li style="margin-bottom:12px"><a href="{a.Link}" style="color:#818cf8;font-weight:600">{a.Title}</a><br/><span style="color:#94a3b8;font-size:0.85rem">{(a.Summary ?? "").Length > 100 ? (a.Summary ?? "")[..100] + "..." : (a.Summary ?? "")}</span><br/><a href="{baseUrl}/api/feeds/unfavorite-from-email?userId={userId}&feedTitle={Uri.EscapeDataString(a.FeedTitle)}" style="color:#64748b;font-size:12px;text-decoration:underline;">Unsubscribe / Remove from favorites</a></li>"""));

        var html = $"""
            <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#1e293b;color:#f1f5f9;padding:2rem;border-radius:16px">
              <h2 style="color:#818cf8">Your Favorite Feed Digest</h2>
              <p style="color:#94a3b8">Here are the latest articles from your favorite feeds:</p>
              <ul style="list-style:none;padding:0">{itemsHtml}</ul>
            </div>
            """;
        ...
        if (string.IsNullOrEmpty(_host) || string.IsNullOrEmpty(_username))
        {
            _logger.LogInformation("[EMAIL] Favorite digest for {Email}: {ArticleCount} articles", email, articles.Count);
            foreach (var a in articles)
                _logger.LogInformation("[EMAIL DIGEST] {Title} — {Link}", a.Title, a.Link);
            return;
        }
        ...
        try
        {
            ...
            using var mail = new MailMessage(_username, email, "Your Favorite Feed Digest — Personal RSS Reader", html)
            {
                IsBodyHtml = true
            };
            await smtp.SendMailAsync(mail);
        ...
        {
            _logger.LogError(ex, "[EMAIL] Failed to send favorite digest to {Email}", email);
        }
    }
}
